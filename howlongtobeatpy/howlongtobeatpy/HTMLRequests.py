...
class HTMLRequests:
    ...
    def get_search_request_data(
        game_name: str, search_modifiers: SearchModifiers, page: int, auth_struct
    ):
        """
    ...
    def send_web_request(
        ...
        if search_info_data is not None and search_info_data.search_url is not None:
            auth_struct = HTMLRequests.send_website_get_auth_token(
                search_info_data.search_url, request_user_agent
            )
        else:
            auth_struct = HTMLRequests.send_website_get_auth_token(
                None, request_user_agent
            )
        # Make the request
        headers = HTMLRequests.get_search_request_headers(
            auth_struct, request_user_agent
        )
        if search_info_data is not None and search_info_data.search_url is not None:
        ...
        payload = HTMLRequests.get_search_request_data(
            game_name, search_modifiers, page, auth_struct
        )
        resp = requests.post(
    ...
    async def send_async_web_request(
        ...
        auth_struct = None
        if search_info_data is not None and search_info_data.search_url is not None:
            auth_struct = await HTMLRequests.async_send_website_get_auth_token(
                search_info_data.search_url, request_user_agent, session
            )
        else:
            auth_struct = await HTMLRequests.async_send_website_get_auth_token(
                None, request_user_agent, session
            )
        # Make the request
        headers = HTMLRequests.get_search_request_headers(
            auth_struct, request_user_agent
        ...
        async with HTMLRequests.get_session(session) as session:
            async with session.post(
                HTMLRequests.SEARCH_URL, headers=headers, data=payload, timeout=timeout
            ) as resp_with_key:
    ...
    def get_title_request_headers(user_agent):
        ...
        @return: The headers object for the request
        """
        headers = {"User-Agent": user_agent, "referer": HTMLRequests.REFERER_HEADER}
        return headers
    ...
    async def async_send_website_request_getcode(
        ...
        async with HTMLRequests.get_session(session) as session:
            async with session.get(
                HTMLRequests.BASE_URL, headers=headers, timeout=timeout
            ) as resp:
                if resp is not None and resp.status == 200:
    ...
    def send_website_get_auth_token(parsed_search_url, user_agent):
        ...
        if parsed_search_url is not None:
            auth_token_url = (
                HTMLRequests.BASE_URL
                + parsed_search_url
                + auth_struct.search_url_endpoint
            )
        else:
            auth_token_url = (
                HTMLRequests.BASE_URL
                + auth_struct.search_url
                + auth_struct.search_url_endpoint
            )
    ...
    async def async_send_website_get_auth_token(
        parsed_search_url, user_agent, session: aiohttp.ClientSession | None = None
    ):
        """
        Function that send a request to howlongtobeat to get the x-auth-token to get in the request
        @return: The auth token to use
